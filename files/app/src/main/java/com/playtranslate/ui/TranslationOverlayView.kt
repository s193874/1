...
class TranslationOverlayView(
    ...
    private fun rebuildChildren() {
        ...
        measured.zip(resolved).forEach { (box, resolvedBox) ->
            ...
            if (box.displayMode == TextBoxDisplayMode.BILINGUAL_NOTE) {
                val gap = 2f * dp
                val noteMinW = (88f * dp).toInt()
                val noteMaxW = (width - 8f * dp).toInt().coerceAtLeast(1)
                val noteW = rect.width().toInt().coerceAtLeast(noteMinW).coerceAtMost(noteMaxW)
                val noteH = (rect.height() * 0.58f).toInt()
                    .coerceAtLeast((26f * dp).toInt())
                    .coerceAtMost((64f * dp).toInt())
                val noteLeft = rect.left
                    .coerceIn(4f * dp, (width - noteW - 4f * dp).coerceAtLeast(4f * dp))
                val below = rect.bottom + gap
                val noteTop = if (below + noteH <= height - 4f * dp) {
                    below
                } else {
                    (rect.top - noteH - gap).coerceAtLeast(4f * dp)
                }
                val noteRect = RectF(noteLeft, noteTop, noteLeft + noteW, noteTop + noteH)

                val child: View = if (box.translatedText.isEmpty()) {
                    buildSkeletonView(
                        noteW, noteH, 1,
                        Color.argb(185, 0, 0, 0), Color.WHITE,
                    ...
                    OutlinedTextView(context).apply {
                        ...
                        setPadding(textMargin * 2, textMargin, textMargin * 2, textMargin)
                        ...
                        contentDescription = if (box.sourceText.isBlank()) {
                            box.translatedText
                        } else {
                            "${box.sourceText} — ${box.translatedText}"
                        }
                    }
                }
                child.setTag(R.id.tag_bg_color, Color.BLACK)
                addView(child, LayoutParams(noteW, noteH).apply {
                    leftMargin = noteRect.left.toInt()
                    topMargin = noteRect.top.toInt()
                })
                return@forEach
            }
