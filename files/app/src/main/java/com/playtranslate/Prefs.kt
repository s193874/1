...
class Prefs internal constructor(
    ...
    var sourceLang: String
        get() = sp.getString(KEY_SOURCE_LANG, TranslateLanguage.ENGLISH) ?: TranslateLanguage.ENGLISH
    ...
    /** Background style for the translation overlay boxes — adaptive
     *  (sampled from game content, upstream default), or a forced
     *  dark/light card. Applied by [com.playtranslate.ui.OverlayBoxStyle]. */
    var overlayBoxColorMode: com.playtranslate.ui.OverlayBoxColorMode
        get() = com.playtranslate.ui.OverlayBoxColorMode.fromKey(
            sp.getString(KEY_OVERLAY_BOX_COLOR_MODE, null))
        set(v) = sp.edit { putString(KEY_OVERLAY_BOX_COLOR_MODE, v.storageKey) }

    /** Background opacity for the translation overlay boxes (0.3–1.0). */
    var overlayBoxOpacity: Float
        get() = sp.getFloat(KEY_OVERLAY_BOX_OPACITY, DEFAULT_OVERLAY_BOX_OPACITY)
            .coerceIn(MIN_OVERLAY_BOX_OPACITY, 1f)
        set(v) = sp.edit {
            putFloat(KEY_OVERLAY_BOX_OPACITY, v.coerceIn(MIN_OVERLAY_BOX_OPACITY, 1f))
        }
    ...
    companion object {
        ...
        const val DEFAULT_CAPTURE_INTERVAL_SEC = 0.8f

        const val KEY_OVERLAY_BOX_COLOR_MODE = "overlay_box_color_mode"
        const val KEY_OVERLAY_BOX_OPACITY    = "overlay_box_opacity"
        const val MIN_OVERLAY_BOX_OPACITY = 0.3f
        /** Matches the alpha the sampled adaptive colour historically carried
         *  (≈200/255) — the ADAPTIVE + default-opacity fast path in
         *  [com.playtranslate.ui.OverlayBoxStyle.applyTo] leaves boxes
         *  untouched, so keep this aligned with that legacy look. */
        const val DEFAULT_OVERLAY_BOX_OPACITY = 0.8f
